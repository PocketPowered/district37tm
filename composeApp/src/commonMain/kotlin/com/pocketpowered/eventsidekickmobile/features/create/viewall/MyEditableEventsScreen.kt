...
/**
 * Screen that displays all editable events (owned + collaborated) for the current user
 */
@Composable
fun MyEditableEventsScreen(
    onBackClick: () -> Unit = {}
) {
    ...
    val config = remember {
        PaginatedListConfig<Event>(
            title = "My Events",
            subtitle = "Events you own or collaborate on",
            initialItems = emptyList(),
            totalCount = 0,
            initialCursor = null,
            emptyMessage = "No events yet. Create your first event or get invited to collaborate!"
        )
    }

    // Create the data source
    val dataSource = remember {
        MyEditableEventsPaginationDataSource(createHubRepository)
    }
