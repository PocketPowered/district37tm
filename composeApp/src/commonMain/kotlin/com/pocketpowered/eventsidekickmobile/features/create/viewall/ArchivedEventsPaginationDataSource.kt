...
/**
 * Pagination data source for loading user's archived events
 */
class ArchivedEventsPaginationDataSource(
    ...
    override suspend fun loadMore(cursor: String?): PaginationResult<Event> {
        return when (val result = createHubRepository.getMyArchivedEvents(cursor)) {
            is Resource.Success -> {
            ...
            is Resource.Error -> {
                throw Exception(result.message ?: "Failed to load archived events")
            }
            else -> {
                throw Exception("Unexpected state while loading archived events")
            }
        }
    }
}
