...
import com.district37.toastmasters.features.create.event.steps.EventDateStep
import com.district37.toastmasters.features.create.event.steps.EventDescriptionStep
import com.district37.toastmasters.features.create.event.steps.EventNameStep
import com.district37.toastmasters.features.create.event.steps.EventTypeStep
import com.district37.toastmasters.features.create.event.steps.VenueSelectionStep
import com.district37.toastmasters.util.Resource
import com.district37.toastmasters.util.toImageBitmap
import org.koin.compose.viewmodel.koinViewModel

/**
 * Multi-step wizard screen for creating a new event.
 * Guides the user through collecting event information step by step.
 *
 * @param viewModel The wizard view model
 * @param onEventCreated Callback when event is successfully created with the new event's ID
 * @param onCancel Callback when user cancels or exits the wizard
 * @param onCreateVenue Callback to navigate to venue creation flow
 * @param createdVenueId Optional ID of a venue that was just created (from nested flow)
 */
@Composable
fun CreateEventWizardScreen(
    viewModel: CreateEventWizardViewModel = koinViewModel(),
    onEventCreated: (eventId: Int) -> Unit,
    onCancel: () -> Unit,
    onCreateVenue: () -> Unit,
    createdVenueId: Int? = null
) {
    ...
    // Handle newly created venue from nested flow
    LaunchedEffect(createdVenueId) {
        createdVenueId?.let { venueId ->
            viewModel.loadAndSelectVenue(venueId)
        }
    }
    ...
    val nextLabel = if (isLastStep) "Create Event" else "Continue"
    ...
    WizardScaffold(
        ...
        when (currentStep) {
            ...
            4 -> VenueSelectionStep(
                viewModel = viewModel,
                onCreateVenueClick = onCreateVenue
            )
            5 -> {
